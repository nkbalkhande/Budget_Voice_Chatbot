...
index_name = "langchain-rag-pinecone"

# Streamlit UI setup
st.set_page_config(
    page_title="🗣️ Multilingual Budget Chatbot", layout="centered")
st.title("📊 Indian Budget Voice Chatbot (with Translation)")
st.markdown(
    "Upload a Budget PDF, speak/type your question in your language, and hear the answer.")
...
selected_language = st.selectbox(
    "🌐 Choose your language:", list(language_codes.keys()))
...
"""
    ...
    if st.button("🎤 Speak Your Question"):
        ...
        try:
            user_voice = recognizer.recognize_google(
                audio, language=lang_code + "-IN")
            st.success(f"🗣️ You asked: {user_voice}")

            # Translate to English
            translated_query = GoogleTranslator(
                source=lang_code, target="en").translate(user_voice)
            ...
            translated_answer = GoogleTranslator(
                source="en", target=lang_code).translate(answer_en)
            ...
            with open(audio_path, "rb") as af:
                ...
                st.markdown(
                    f'<audio autoplay controls src="data:audio/mp3;base64,{b64}"></audio>', unsafe_allow_html=True)
    ...
    if user_query:
        with st.spinner("Translating and answering..."):
            translated_q = GoogleTranslator(
                source=lang_code, target="en").translate(user_query)
            response = qa_chain.invoke({"query": translated_q})
            translated_a = GoogleTranslator(
                source="en", target=lang_code).translate(response["result"])
